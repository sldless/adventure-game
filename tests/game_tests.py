...
def test_exit_setup():
    ...
    assert_equal(engine.map.all_exits['tube_to_hall'].destination,
                 'central_hallway_west')
    assert_equal(engine.map.all_exits['tube_to_hall_rev'].destination,
                 'tube_room')
    assert_equal('s' in engine.map.all_exits['tube_to_hall_rev'].keywords, True)
    assert_equal('south' in engine.map.all_exits['tube_to_hall_rev'].keywords,
                 True)
...
def test_room_setup():
    ...
    assert_equal(
        ...
        "You are in an operating room, lined with mysterious person-sized tubes"
    )
...
def test_map_setup():
    ...
    assert_equal(engine.map.all_rooms['kitchen'].items['corpse'].type,
                 'scenery')
